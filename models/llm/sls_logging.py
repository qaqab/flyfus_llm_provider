...
def write_invocation_log(credentials: dict[str, Any], event: dict[str, Any]) -> None:
    ...
    contents = [
        ("log_id", str(event.get("log_id") or event.get("invocation_id") or "")),
        ("request_id", str(event.get("request_id") or event.get("invocation_id") or "")),
        ("x_request_id", str(event.get("x_request_id") or event.get("invocation_id") or "")),
    ...
    try:
        ...
        LogClient(endpoint, access_key_id, access_key_secret).put_logs(
            PutLogsRequest(project, logstore, "flyfus-llm-provider", "", [log_item])
        )
